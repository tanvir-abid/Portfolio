...
Bird.Tri = Bird.def(
  function(p1, p2, p3) {
    ...
    draw: function() {
      ...
      var scaleFactor = 2;
        // Apply the scaling factor to each vertex
  this.Vtxs.forEach(function(vertex) {
    vertex.x *= scaleFactor;
    vertex.y *= scaleFactor;
    vertex.z *= scaleFactor;
  });
      var col = this.col();
      // Draw the triangle with the scaled vertices
  Bird.$.fillStyle = col;
  ...
  Bird.$.stroke();
    ...
    col: function() {
      ...
      x1.add(x2);
      x1.add(x3);
      var _r = Math.floor(x1.x * 150);
      var _g = Math.floor(x1.y * 150);
      var _b = Math.floor(x1.z * 150);
      return 'rgb(' + _r + ',' + _g + ',' + _b + ')';
    },
    norm: function() {
      var v1 = this.Vtxs[0];
...
function draw() {
  ...
  Bird.canv = {
    w: c.width = window.innerWidth-15,
  ...
  for (var i = 0; i < 100; i++) {
    _b = b[i] = new Bird.obj();
    _b.pos.x = Math.random() * Bird.canv.w - Bird.canv.w / 2;
    _b.pos.y = Math.random() * Bird.canv.h - Bird.canv.h / 2;
    _b.pos.z = Math.random() * 800 - 400;
    _b.vel.x = Math.random() * 4 - 2; // Adjusted velocity range for x-axis
    _b.vel.y = Math.random() * 4 - 2; // Adjusted velocity range for y-axis
    _b.vel.z = Math.random() * 4 - 2; // Adjusted velocity range for z-axis
    _b._coll(true);
  ...
  document.addEventListener('mousemove', function (event) {
    mouseX = event.clientX - Bird.canv.w / 2;
    mouseY = (Bird.canv.h / 2 - event.clientY);
  });
  ...
  function draw() {
    ...
    b.forEach(function(e, i, a) {
      var _b = b[i];
      var mouseVector = new Bird.Vtr(mouseX, mouseY, 0);
    _b.rep(mouseVector); 
      _b.run(b);
      var bird = birds[i];
